...
export default function DiemDanhChiTietPage({ params }: { params: Promise<{ maSuKien: string }> }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/diemdanh/get/all/${maSuKien}?${query.toString()}`, {
        credentials: 'include'
      });
  ...
  const handleViewDetails = async (maDiemDanh: string) => {
    ...
    try {
      const res = await fetch(`/api/diemdanh/get/${maDiemDanh}`, {
        credentials: 'include',
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleMarkAttendance = async () => {
    ...
    setMarkingError('');
    try {
      const res = await fetch(`/api/diemdanh/${selectedDiemDanh.maDiemDanh}`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json();
