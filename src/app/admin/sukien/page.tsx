...
export default function SuKienTablePage() {
  ...
  const fetchData = async () => {
    const url = new URL('/api/sukien/get/all')
  ...
  const onSubmitAddForm = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/sukien/add', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/sukien/update/${editForm.maSuKien}`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
    ...
    <div>
      ...
      <div className="modal-buttons" style={{ marginTop: '20px' }}>
        <button
          ...
          onClick={async () => {
            try {
              const res = await fetch(`/api/sukien/cancel/${selectedCancel.maSuKien}`, {
                method: 'PUT',
                credentials: 'include',
              });
